<%@ page language="java" pageEncoding="UTF-8"%>
<%@ include file="../block/block_taglibs.jsp" %>
<tiles:insert beanName="indexPage">
	<tiles:put name="title" value="招商" />
	<tiles:put name="body" type="String">
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>招商管理</title>
		<c:if test="${merchants.state == 0}">
	<script src="${ctx}/js/hgl/district.js"></script>
	</c:if>
	<%@include file="merchantsStyle.jsp"%>
	<script src="${ctx}/js/jquery/uploadPreview.min.js"></script>
	<script src="${ctx}/js/jquery/jquery.ui.widget.js"></script>
	<script src="${ctx}/js/jquery/jquery.fileupload.js"></script>
	<script src="${ctx}/js/jquery/jquery.iframe-transport.js"></script>
	<script src="${ctx}/js/jquery/jquery.fileupload-process.js"></script>
	<script src="${ctx}/js/jquery/jquery.fileupload-validate.js"></script>
</head>
<body class="skin-blue fixed">
<div class="wrapper">
  <div class="content-wrapper">
  	<section class="content-header">
      <h1>
        我的主页
        <small>招商详情修改</small>
      </h1>
    </section>
    <section class="content">
      <div class="row no-margin">
    		<div class="box box-primary">
    		<div class="p8">
              <a href="merchants" class="btn btn-primary">返回列表</a>
         	</div>
    			 <div class="p10" style="padding-left: 35px;">
               <form id="updateMerchants" action="updateMerchants" class="form-horizontal" method="post">
               <input type="hidden" name="id" value="${merchants.id}"/>
               <input type="hidden" name="state" value="${merchants.state}"/>
               <input type="hidden"  id="provinceCode" value="${merchants.province}"/>
              	<input type="hidden" id="cityCode" value="${merchants.city}"/>
               	<input type="hidden"  id="countryCode" value="${merchants.country}"/>
               	<input type="hidden"  id="streetCode" value="${merchants.street}"/>
                <input type="hidden" name="reserved1" id="reserved1_Path" value="${merchants.reserved1}"/>
                <div class="mt20 clearfix" >
                  	<div class="fl w_120 input_label">品牌：</div> 
                      <div class="fl">
                        <select id="brandid" name="brandid" class="select">
                        <c:forEach var="item" items="${brands}" varStatus="status">
                          <option value="${item.id}">${item.producttypeName} - ${item.name}</option>
                          </c:forEach>
                        </select>
                      </div>
                  </div>
                 
                  <div class="mt20 clearfix" >
                  	<div class="fl w_120 input_label">招商类型：</div> 
                      <div class="fl">
                        <select id="type" name="type" class="input">
                          <option value="1">金额</option>
                          <option value="2">数量</option>
                        </select>
                      </div>
                  </div>
                   <div class="mt20 clearfix" >
                  	<div class="fl w_120 input_label">招商购买量：</div> 
                      <div class="fl">
                        <input id="number" name="number" type="text" class="input" placeholder="大于0" value="${merchants.number}">
                      </div>
                      <div class="fl msg">
		                    <span class="msg_pic"></span>
		                    <span class="msg_content" title=""></span>
		             </div>
                  </div>
               	<div class="mt20 clearfix" >
                  	<div class="fl w_120 input_label">优惠券：</div> 
                      <div class="fl">
                        <input id="coupons" name="coupons" type="text" class="input" placeholder="大于0" value="${merchants.coupons}">
                      </div>
                      <div class="fl msg">
		                    <span class="msg_pic"></span>
		                    <span class="msg_content" title=""></span>
		             </div>
                  </div>
                
                  <div class="mt20 clearfix" >
                  	<div class="fl w_120 input_label">招商层级：</div> 
                  	<c:if test="${merchants.state != 0}" var="isfabu">
                  		<div class="fl" style="line-height: 32px;">${merchants.levelName}</div>
                  	</c:if>
                  	<c:if test="${!isfabu}">
                      <div class="fl">
                       <select id="level" name="level" class="input" onchange="levelChang();">
                          <option value="2">市</option>
                          <option value="3">区县</option>
                          <option value="4">街道</option>
                        </select>
                      </div>
                     </c:if>
                  </div>
                  <div class="mt20 clearfix" >
                  	<div class="fl w_120 input_label">其他：</div> 
                      <div class="fl">
                       <input id="other" name="other" type="text" class="input" value="${merchants.other}">
                      </div>
                      <div class="fl msg">
		                    <span class="msg_pic"></span>
		                    <span class="msg_content" title=""></span>
		             </div>
                  </div>
                  <div class="mt20 clearfix" >
                  	<div class="fl w_120 input_label">招商区域：</div> 
                  	<c:if test="${merchants.state != 0}" var="isfabu">
                  		<div class="fl" style="line-height: 32px;">${merchants.addressName}</div>
                  	</c:if>
                  	<c:if test="${!isfabu}">
                      <div class="fl">
                       <select id="province" name="province" class="select1" onchange="changeGrade();" value="${merchants.province}">
                       	  
                          <c:forEach  var="item" items="${provinceInfos}" varStatus="status" >
                         
                         	  <option
	                    	<c:if test="${item.id == merchants.province}">
	                    		selected='selected'
	                    	</c:if>
	                 			value="${item.id}">${item.name}</option> 
                          </c:forEach>
                        </select>
                      </div>
                      <div class="fl" style="margin-left: 10px;">
                       <select id="city" name="city" class="select1" onchange="changeCity();" value="${merchants.city}">
                          
                        </select>
                      </div>
                       <div class="fl">
                       <select id="country" name="country" class="select1 ml10" style="display:none;" onchange="changeCountry();">
                           <!-- <option id="countryid"></option> -->
                        </select>
                      </div>
                       <div class="fl">
                       <select id="street" name="street" class="select1 ml10" style="display:none;" >
                        <!--   <option id="streetid"></option> -->
                        </select>
                      </div>
                      <div class="fl msg">
		                    <span class="msg_pic"></span>
		                    <span class="msg_content" title=""></span>
		             </div>
		             </c:if>
                  </div>
                 	   <div class="mt20 clearfix file" >
                  	<div class="fl w_120 input_label">封面：</div> 
                      <div class="col-sm-3">
                           <input type="file" name="imgFile" class="pImg" id="reserved1">
                            <div id="pImgdivUp">
				               <c:if test="${not empty merchants.reserved1}">
			               		 <img id="pImgShowUp" src='generateImage?id=${merchants.id}'/>
			                  </c:if>
			                  <c:if test="${empty merchants.reserved1}">
			               		<img id="pImgShowUp">
			                  </c:if>
			                  
			                  </div>
                           <span class="glyphicon glyphicon-plus"></span>
                       </div>
                       <div class="fl msg">
		                    <span class="msg_pic"></span>
		                    <span class="msg_content" title=""></span>
		             </div>
                  </div>
                    <c:if test="${!isfabu}">
                    <div class="mt20 clearfix" >
                  	<div class="fl w_120 input_label"></div> 
                     <!-- 	<div id="ismsg"></div> -->
                     	<input type="hidden" id="ckMsg" onblur="ckMsg()">
                       <div id="vcodeMsg"  class="fl msg">
		                    <span class="msg_pic"></span>
		                    <span class="msg_content" title=""></span>
		             </div>
                  </div> 
                  </c:if>
               <div class="mt20 clearfix" >
               <div class="fl w_120 input_label"></div> 
                 <div class="fl">
                    <input type="submit" onclick="return confirmAction('确定保存么？', ck_data);" class="btn btn-primary btn-sm"
						value="保存" />

                  </div>
                  <div class="fl" style="margin-left: 20px;">
                   
                    <a href="merchants" class="btn btn-primary btn-sm">取消</a>
                  </div>
                </div>
             </form>
    			 </div>
    		</div>
    	</div>

      
             
         <script>
         
         $(function(){
        		var levelcode ="${merchants.level}";
        		$(".level").val(levelcode); 
        		 lechang(levelcode);
        		
        	 });
         
         function levelChang(){
        	 var level = $("#level option:selected").val();
        	 lechang(level);
        	 
        }
         
         
         $(function(){
   		  new uploadPreview({ UpBtn: "reserved1", DivShow: "pImgdivUp", ImgShow: "pImgShowUp" });
   		 
   		});
   	 $(function(){
   			$(".pImg").fileupload({
   				url : ctx+'/supplier/validateImg',
   			    autoUpload : true,
   			    singleFileUploads : true,
   			    acceptFileTypes : /(\.|\/)(|gif|jpe?g|png|bmp)$/i,
   			    maxFileSize : 5097152,
   			    change: function (e, data) { 
   			        $.each(data.files, function (index, file) {
   			        });
   			    }
   			}).on(
   			        'fileuploaddone',
   			        function(e, data) {
   			         new uploadPreview({ UpBtn: "reserved1", DivShow: "pImgdivUp", ImgShow: "pImgShowUp" });
   				   		  
   			            if (data.result.code == '0'){
   			            	var imgName = data.result.imgNo.split("_");
   			            	$("#"+imgName[0]+"_Path").val(data.result.imgNo);
   			            }else{
   			            
   			            }
   			}).on('fileuploadsubmit', function (e, data) {
   		        data.formData = { imgNo: $(this).attr("id") };  //如果需要额外添加参数可以在这里添加
   		    });
   		});
   	 
         function lechang(level){
        	 if(level == 3){
 					 $("#country").attr("style","display:block;");
 					 $("#street").attr("style","display:none;");
 					countrycc();
         	 }else if(level == 4){
         		 $("#country").attr("style","display:block;");
 					 $("#street").attr("style","display:block;");
 					streetcc();
         	 }else if(level ==2){
         		$("#country").val("0");
         		$("#street").val("0");
         		 $("#country").attr("style","display:none;");
         		 $("#street").attr("style","display:none;");
         		
         	 }
         }
         
         var ck_data = {'number':'','coupons':'','other':'','province':'','city':'','ckMsg':'','reserved1':''};
         function number() {
             var val = $('#number').val();
             if(!is_numeric(val) || val <= 0){
                 ck_data["number"] = "招商购买量不能为空并为正整数";
                 return CK_FALSE;
             } 
             return CK_TRUE;
         };
         function coupons() {
             var val = $('#coupons').val();
             if(!is_numeric(val) || val <= 0){
                ck_data["coupons"] = "优惠卷不能为空并为正整数";
                 return CK_FALSE;
             } 
             return CK_TRUE;
         };
         function other() {
             var val = $('#other').val();
             if(!check_length(val,0,100)){
                ck_data["other"] = "字数不能超过100";
                 return CK_FALSE;
             } 
             return CK_TRUE;
         };
         
         function countrycc() {
             var val = $('#country').val();
             if(!is_numeric(val) || val <= 0){
                // alert("优惠卷不能为空并为正整数");
                ck_data["country"] = "请选择招商区域";
                 return CK_FALSE;
             } 
             return CK_TRUE;
         };
         function reserved1() {
             var val = $('#reserved1_Path').val();
             if(val==""){
                ck_data["reserved1"] = "请上传招商封面图";
                 return CK_FALSE;
             } 
             return CK_TRUE;
         };
         function province() {
             var val = $('#province').val();
             if(!is_numeric(val) || val <= 0){
                ck_data["street"] = "请选择招商区域";
                 return CK_FALSE;
             } 
             return CK_TRUE;
         };
         
         function city() {
             var val = $('#city').val();
             if(!is_numeric(val) || val <= 0 || val==null){
                ck_data["street"] = "请选择招商区域";
                 return CK_FALSE;
             } 
             return CK_TRUE;
         };
         
         function streetcc() {
             var val = $('#street').val();
             if(!is_numeric(val) || val <= 0){
                // alert("优惠卷不能为空并为正整数");
                ck_data["street"] = "请选择招商区域";
                 return CK_FALSE;
             } 
             return CK_TRUE;
         };
         
         function ckMsg(){
        	 var isckMsg= true;
         	 var val = $('#brandid').val();
         	 var val1 = $('#city').val();
         	 var val2 = $('#country').val();
         	 var val3 = $('#street').val();
         	 if(val3!=null&&is_numeric(val3) && val3 >= 0){ 
         	   $.ajax({
     		        type: "POST",
     		        async: false ,
     		        url:ctx+'/merchants/checkMerchants?brandId='+val+"&city="+val1+"&country="+val2+"&street="+val3+"&merchantsId="+"${param.id}",
     		        success: function(response){
     		           if(response==false){
     		        	   isckMsg =false;
     		           }else{    
     		        	   isckMsg = true;
     		           }
     		        }
     		    });
         	 }
         	if(isckMsg== false){
         		// f_ck_error($("#vcodeMsg"), "此品牌招商区域的招商已存在！！"); 
         		 ck_data["ckMsg"] = "此品牌招商区域的招商已存在！！";
                   return CK_FALSE;
               } 
               return CK_TRUE;   
         }
         
         function ismsg() {
         	/* ckMsg(); */
         	 if(isckMsg== false){
         		// f_ck_error($("#vcodeMsg"), "此品牌招商区域的招商已存在！！"); 
         		 ck_data["ismsg"] = "此品牌招商区域的招商已存在！！";
                   return CK_FALSE;
               } 
               return CK_TRUE;       
           
        };
         
         $("#brandid").val("${merchants.brandid}"); 
         $("#type").val("${merchants.type}");
         $("#level").val("${merchants.level}");
         $("#address").val("${merchants.address}");
       
         
     </script>
         
    </section>
  </div>
</div>
</body>
</html>
</tiles:put>
</tiles:insert>