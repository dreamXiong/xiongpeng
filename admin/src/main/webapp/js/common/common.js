...
/**
 * 时间比较，
 * @param dateOne 格式"yyyy-mm-dd"
 * @param dateTwo 格式"yyyy-mm-dd"
 * @returns {Boolean}
 */
...
/**
 * @deprecated 用于比较固定格式（yyyy-mm-dd）日期的天数差
 * @param dateOne 格式"yyyy-mm-dd"
 * @param dateTwo 格式"yyyy-mm-dd"
 * @returns {Number} 天数差； 例如：subTime("2013-10-15","2013-10-22");将返回-7
 */
function subTime(dateOne, dateTwo) {
    ...
    return (dateOne - dateTwo) / (24 * 60 * 60 * 1000);
}
    ...
    return {
        ...
        initForm : function(funFlag, options) {
            ...
            $("#key_" + funFlag + "_save").bind("click", function() {
                ...
                if (!validateForm("key_" + funFlag + "_editFrm"))
                {
                    return false;
                }
...
function submitIn(id, removeClassName, addClassName) {
    $("#" + id).removeClass(removeClassName);
    $("#" + id).addClass(addClassName);
    $("#" + id).removeAttr("onclick");
};
function submitAfter(id, removeClassName, addClassName, functionName) {
    $("#" + id).removeClass(removeClassName);
    $("#" + id).addClass(addClassName);
