...
var PopupLayer = new Class({
	...
	_init:function(options){
        ...
        var trigger = $(this.options.trigger),popupBlk = $(this.options.popupBlk);
        if(trigger.length == 0){
            popupBlk.addClass("none");
            return;
        }
		...
		this.trigger = trigger;                         //把触发元素封装成实例的一个属性，方便使用及理解
		this.popupBlk = popupBlk;                       //把弹出内容层元素封装成实例的一个属性
	...
	doEffects:function(way){
        ...
        var leftValue =(($(document).width() - this.popupLayer.width())/2) + "px",
            topValue =(document.documentElement.clientHeight - this.popupLayer.height())/2 + $(document).scrollTop();
        this.popupLayer.css({left:leftValue,top:topValue,opacity:1.0});
        way == "open"?this.popupLayer.show():this.popupLayer.hide();
	    /*
        if(way == "open"){
            this.popupLayer.css({opacity:0.3}).show(400,function(){
                this.popupLayer.animate({
                    ...
                    opacity:0.8
                },800,function(){this.popupLayer.css("opacity",1)}.binding(this));
            }.binding(this));
        }else{
            this.popupLayer.animate({
    ...
    open:function(way){
        this.options.useOverlay?this.overlay.show():null;
        this.options.useFx?this.doEffects("open"):this.popupLayer.show();
    },
...
$(document).ready(function() {
	var t9 = new PopupLayer({trigger:".gwc",popupBlk:"#p1",closeBtn:"#close1",useOverlay:true,useFx:true,
		offsets:{
			x:0,
			y:-41
		}
