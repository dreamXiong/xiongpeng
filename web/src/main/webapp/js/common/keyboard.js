 ...
 *   - Chinese password keyboard By masterjames
 */
...
(function() {
    ...
    this.VKI_kts = this.VKI_kt = "\u5feb\u8bfa\u8fc5\u6c47"; // Default keyboard layout
    this.VKI_langAdapt = true; // Use lang attribute of input to select keyboard
    this.VKI_size = 3; // Default keyboard size (1-5)
    this.VKI_sizeAdj = true; // Allow user to adjust keyboard size
    this.VKI_ClearPasswords = false; // Clear password fields on focus
    //    this.VKI_imageURI = "https://www.shinpay.com/styles/images/keyboard.png"; // If empty string, use imageless mode
    this.VKI_clickless = 0; // 0 = disabled, > 0 = delay in ms
    this.VKI_activeTab = 0; // Tab moves to next: 1 = element, 2 = keyboard enabled element
    this.VKI_enterSubmit = false; // Submit forms when Enter is pressed
    this.VKI_keyCenter = 3;
    this.VKI_passwordPad = true;//change key position
    ...
    //    this.projectName = pathName.substring(0,
    ...
    if (this.VKI_passwordPad)
    {
        ...
        var pos = Math.ceil(Math.random() * 10);
        for ( var i = 0; i < 10; i++)
        {
            if (pos + i < 10)
                newKey[i] = key[pos + i];
            else
                newKey[i] = key[pos + i - 10];
        ...
        var newCharKey = new Array();

        pos = Math.ceil(Math.random() * 10);
        for ( var i = 0; i < 26; i++)
        {
            if (pos + i < 26)
                newCharKey[i] = charKey[pos + i];
            else
                newCharKey[i] = charKey[pos + i - 26];

        }

        this.VKI_layout['\u5feb\u8bfa\u8fc5\u6c47'] = {
            ...
            'keys' : [
                    [["`", "~"], newKey[0], newKey[1], newKey[2], newKey[3],
                            ...
                            newKey[8], newKey[9], ["-", "_", "\u00a5"],
                            ["=", "+", "\u00d7", "\u00f7"], ["退格", "Bksp"]],
                    [["Tab", "Tab"], newCharKey[0], newCharKey[1],
                            newCharKey[2], newCharKey[3], newCharKey[4],
                            newCharKey[5], newCharKey[6], newCharKey[7],
                            newCharKey[8], newCharKey[9], ["[", "{", "\u00ab"],
                            ["]", "}", "\u00bb"],
                            ["\\", "|", "\u00ac", "\u00a6"]],
                    [["Caps", "Caps"], newCharKey[10], newCharKey[11],
                            newCharKey[12], newCharKey[13], newCharKey[14],
                            newCharKey[15], newCharKey[16], newCharKey[17],
                            newCharKey[18], [";", ":", "\u00b6", "\u00b0"],
                            ["'", '"', "\u00b4", "\u00a8"], ["确定", "Enter"]],
                    [["Shift", "Shift"], newCharKey[19], newCharKey[20],
                            newCharKey[21], newCharKey[22], newCharKey[23],
                            newCharKey[24], newCharKey[25],
                            [",", "<", "\u00e7", "\u00c7"], [".", ">"],
                            ["/", "?", "\u00bf"], ["Shift", "Shift"]],
                    [[" ", " ", " ", " "], ["Alt", "Alt"]]],
            'lang' : ["en"]
    ...
    */
    ...
    VKI_attach = function(elem) {
        ...
        if (self.VKI_imageURI)
        {
            ...
            elem.parentNode.insertBefore(keybut, (elem.dir == "rtl") ? elem
                    : elem.nextSibling);
        } else
        {
    ...
    if (!this.VKI_layout[this.VKI_kt])
        return alert('没有这个键 "' + this.VKI_kt + '"');
    ...
    for (ktype in this.VKI_layout)
    {
        if (typeof this.VKI_layout[ktype] == "object")
        {
            ...
            for ( var x = 0; x < this.VKI_layout[ktype].lang.length; x++)
                this.VKI_langCode[this.VKI_layout[ktype].lang[x].toLowerCase()
                        .replace(/-/g, "_")] = ktype;
    ...
    if (this.VKI_numberPad)
    {
        ...
        span.appendChild(document.createTextNode("\u6570"));
    ...
    this.VKI_kbsize = function(e) {
        ...
        self.VKI_keyboard.className = self.VKI_keyboard.className.replace(
                / ?keyboardInputSize\d ?/, "");
        if (self.VKI_size != 2)
    ...
    if (this.VKI_sizeAdj)
    {
        ...
        small.appendChild(document.createTextNode(this.VKI_isIElt8 ? "\u5c0f"
                : "\u5c0f"));
        ...
        VKI_mouseEvents(big);
        big.appendChild(document.createTextNode(this.VKI_isIElt8 ? "\u5927"
                : "\u5927"));
        th.appendChild(big);
    ...
    if (this.VKI_deadBox)
    {
        ...
        checkbox.title = this.VKI_i18n['03']
                + ": "
                + ((this.VKI_deadkeysOn) ? this.VKI_i18n['04']
                        : this.VKI_i18n['05']);
        checkbox.defaultChecked = this.VKI_deadkeysOn;
        VKI_addListener(checkbox, 'click', function() {
            this.title = self.VKI_i18n['03']
                    + ": "
                    + ((this.checked) ? self.VKI_i18n['04']
                            : self.VKI_i18n['05']);
            self.VKI_modify("");
            return true;
        }, false);
        ...
        if (self.VKI_deadkeysOn.checked && self.VKI_dead)
        {
            if (self.VKI_dead != character)
            {
                if (character != " ")
                {
                    ...
                    {
                        self
                                .VKI_insert(self.VKI_deadkey[self.VKI_dead][character]);
                        done = true;
                    }
                } else
                {
    ...
    this.VKI_buildKeys = function() {
        ...
        var container = this.VKI_keyboard.tBodies[0]
                                    ...
                                    if (self.VKI_activeTab)
                                    {
                                        if (self.VKI_target.form)
                                        {
                                            ...
                                            for ( var z = 0, me = false, j = -1; z < elems.length; z++)
                                            {
                                                if (j == -1
                                                        && elems[z]
                                                                .getAttribute("VKI_attached"))
                                                    j = z;
                                                if (me)
                                                {
                                                    if (self.VKI_activeTab == 1
                                                            && elems[z])
                                                        break;
                                                    if (elems[z]
                                                            .getAttribute("VKI_attached"))
                        ...
                        VKI_addListener(
                                ...
                                function() {
                                            ...
                                            && !self.VKI_target.readOnly)
                                    {
                                        ...
                                        if (rng[0] < rng[1])
                                            rng[0]++;
                                        self.VKI_target.value = self.VKI_target.value
                                                .substr(0, rng[0] - 1)
                                                + self.VKI_target.value
                                                        .substr(rng[1]);
                                            ...
                                            && !self.VKI_target.readOnly)
                                    {
                                        ...
                                        {
                                            self.VKI_target.range = document.selection
                                                    .createRange();
                                        }
                                        if (!self.VKI_target.range.text.length)
                                            self.VKI_target.range.moveStart(
                                                    'character', -1);
                    ...
                    case "Enter":
                        VKI_addListener(
                                ...
                                function() {
                                    ...
                                    {
                                        if (self.VKI_enterSubmit
                                                && self.VKI_target.form)
                                        {
        ...
        if (this.VKI_deadBox)
            this.VKI_deadkeysOn.style.display = (hasDeadKey) ? "inline"
                    : "none";
        if (this.VKI_isIE6)
        {
    ...
    this.VKI_modify = function(type) {
        ...
        var tables = this.VKI_keyboard.tBodies[0].getElementsByTagName('div')[0]
                ...
                switch (lkey[1]) {
                    ...
                    default:
                        if (type)
                        {
                            ...
                            {
                                var text = this.VKI_symbol[lkey[vchar]]
                                        .split("\n");
                                var small = document.createElement('small');
                                ...
                                {
                                    if (z)
                                        small.appendChild(document
                                                .createElement("br"));
                                    small.appendChild(document
                                            .createTextNode(text[z]));
                                }
                                tds[y].appendChild(small);
                            } else
                                tds[y].appendChild(document
                                        .createTextNode(lkey[vchar] || "\xa0"));
                        }
                        if (this.VKI_deadkeysOn.checked)
                        {
                            var character = tds[y].firstChild.nodeValue
                                    || tds[y].firstChild.className;
                            if (this.VKI_dead)
                            {
    ...
    this.VKI_insert = function(text) {
                ...
                || this.VKI_target.value.length < this.VKI_target.maxlength)
        {
            if (this.VKI_target.setSelectionRange && !this.VKI_target.readOnly
                    && !this.VKI_isIE)
            {
                var rng = [this.VKI_target.selectionStart,
                        this.VKI_target.selectionEnd];
                this.VKI_target.value = this.VKI_target.value.substr(0, rng[0])
                        + text + this.VKI_target.value.substr(rng[1]);
                if (text == "\n" && this.VKI_isOpera)
                    rng[0]++;
                this.VKI_target.setSelectionRange(rng[0] + text.length, rng[0]
                        + text.length);
            } else if (this.VKI_target.createTextRange
                    && !this.VKI_target.readOnly)
            {
                try
                {
    ...
    this.VKI_show = function(elem) {
        $(elem).removeClass("inputNotNull password");
        if (!this.VKI_target)
        {
            ...
            if (this.VKI_langAdapt && this.VKI_target.lang)
            {
                ...
                for ( var x = 0, chg = false; !chg
                        && x < this.VKI_langCode.index.length; x++)
            ...
            if (this.VKI_isIE)
            {
                if (!this.VKI_target.range)
                {
                    ...
                    this.VKI_target.range.moveStart('character',
    ...
    this.VKI_position = function(force) {
        if (self.VKI_target)
        {
            ...
            if (force !== true)
            {
                if (kPos[1] + self.VKI_keyboard.offsetHeight - sDis[1]
                        - wDim[1] > 0)
                {
                    place = true;
            ...
            if (place || force === true)
            {
                ...
                while (scr = scr.parentNode)
                {
                    ...
                    if (scr.scrollHeight > scr.offsetHeight
                            || scr.scrollWidth > scr.offsetWidth)
                    {
                        ...
                        var pPos = VKI_findPos(scr), oTop = iPos[1] - pPos[1], oLeft = iPos[0]
                                - pPos[0];
                        var top = oTop + self.VKI_target.offsetHeight;
                        var left = oLeft + self.VKI_target.offsetWidth;
                        var bottom = scr.offsetHeight - oTop
                                - self.VKI_target.offsetHeight;
                        var right = scr.offsetWidth - oLeft
                                - self.VKI_target.offsetWidth;
                        self.VKI_keyboard.style.display = (top < 0 || left < 0
                                || bottom < 0 || right < 0) ? "none" : "";
                        if (self.VKI_isIE6)
                            self.VKI_iframe.style.display = (top < 0
                                    || left < 0 || bottom < 0 || right < 0) ? "none"
                                    : "";
                    }
                }
                self.VKI_keyboard.style.top = iPos[1]
                        - ((self.VKI_target.keyboardPosition == "fixed"
                                && !self.VKI_isIE && !self.VKI_isMoz) ? sDis[1]
                                : 0) + fudge + "px";
                self.VKI_keyboard.style.left = Math.max(10, Math.min(wDim[0]
                        - self.VKI_keyboard.offsetWidth - 25, iPos[0]))
                        + "px";
                if (self.VKI_isIE6)
                {
                    self.VKI_iframe.style.width = self.VKI_keyboard.offsetWidth
                            + "px";
    ...
    this.VKI_close = VKI_close = function() {
        if (this.VKI_target)
        {
            ...
            this.VKI_target.focus();
            if ($(this.VKI_target).hasClass("validatepw"))
            {
                $(this.VKI_target).addClass("inputNotNull password");
                $(this.VKI_target).blur();
            }
            if (this.VKI_isIE)
            {
    ...
    function VKI_scrollDist() {
        ...
        {
            return [html.scrollLeft + document.documentElement.scrollLeft,
