...
<!-- 登录logo开始 -->
	<div class="user-head">
		<div class="area">
			<h1 class="pull-left">
				<a href="http://www.hgeili.com/login/index">
	...
	<div class="register">
		<div class="area">
			...
			<div class="register-nav line-height">
				...
				<form action="jxregister" method="post" enctype="multipart/form-data">
			           ...
			           <input type="hidden" name="organizationImage" id="organizationImage1_Path"/>
			             <input type="hidden" name="logoName" id="logoName1"/>
					...
					<div class="form-col">
						<div >
							<label for="">产品大类</label>
								<select name="productTypeId" style="width:330px">
									<c:forEach var="item" items="${tbProductTypeList}" varStatus="p">
								   		<option value="${item.id }">${item.name }</option>   
								   	</c:forEach>
								  </select> 
							<!-- <input type="text" id="scope" name="scope"> -->
					...
					<div class="form-col">
						...
						<c:if test="${param.recommendCode !=null}" var="ishascode">
							<input type="text"  name="recommendCode" value="${param.recommendCode}" readOnly="true" >	
						</c:if>
						<c:if test="${!ishascode}">
						<input type="text" id="recommendCode" name="recommendCode" value="${param.recommendCode}" >	
						</c:if>
						</div>
						<div  id="rcodeMsg" class="posi msg">
							<span class="msg_pic "></span> 
							<span class="msg_content" title="">可填可不填</span>
						</div>
					</div>
					...
					<div class="form-col">
						...
						<input class="text text-center" type="button" id="btnSendCode" style="background:#f2f2f2;font-size:12px;padding-left:0;line-height:36px;width" onclick="vcodePhoneGet('经销商');" value="获取验证码"/>
						<span class="text-red">*</span>
						</div>
					...
					<div class="show-bg three none merchant-three">
						...
						<ul class="slide-list clear">
							<li>
								...
								<p>和惠给利合作了将近10年，全部正品和贴心服务，是我选择惠给利的理由。</p>
							</li>
							<li>
								<h3 class="text-red text-center margin-bottom">招商渠道</h3>
								<p>惠给利平台会在已经建立市场的区域发布招商，经销商在获得平台认证以后将可以参与招商，在招商的时候需要完成规定的购货量。</p>
...
<script type="text/javascript">
    var ck_data = {'shopName':'','street':'','scope':'','shopImage3':'','name':'','userName':'','password':'','qrpassword':'','recommendCode':'','mobile':'','vcodeImage':'','vcodePhone':'','cbAgreement':''};
    if (!is_null('${errmsg}')) {
    	var msg = '${errmsg}';
