		...
		<div class="area details">
			<div class="navigate">
				<ul class="clear">
					...
					<li><a href="${ctx}/merchnotice/queryMerchNotices">列表页></a></li>
				</ul>
			</div>
			<div>
			<div class="pull-left details-left">
				<article>
					<header>
						<h3 class="text-center">${merchNotice.name}</h3>
						<p class="text-right"><small>发布时间：
							<jsp:useBean id="createddatetime" class="java.util.Date"/>  
    						<jsp:setProperty name="createddatetime" property="time" value="${merchNotice.createddatetime}" />  
    						<fmt:formatDate value="${createddatetime}" type="both" pattern="yyyy-MM-dd HH:mm:ss" />
						</small></p>
					</header>
					<section style="line-height: 2em">${merchNotice.detail}</section>					
				</article>
			</div>
