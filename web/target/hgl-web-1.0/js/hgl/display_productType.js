function firstProductType(id,name){
	...
	$("#mainName").val(name);
	 $.ajax({
         type:"POST",
         url : ctx+"/brand/display_linkageMainPage",
         data:{
             id:id
         },
         dataType:"json",
         success:function(response){
        	 ...
        	 if(len > 0){
             	...
             	$("#secondForAjax").append( "<li class='"+clearfix+"' onclick='scoendProductType("+response.secondList[i].id+","+'"'+response.secondList[i].name+'"'+");'>"
             										...
             										+"<a>"+response.secondList[i].name+"("+response.secondList[i].id +")"+"</a>"
          	...
          	if(tLen > 0){
          		...
          		for(var i=0; i<tLen; i++){
	          		$("#thirdForAjax").append( "<li class='clearfix'>"
	          										...
	          										+"<a>"+response.thirdList[i].name+"("+response.thirdList[i].id+")"+"</a>"
...
function scoendProductType(id,name){
	...
	$("#parentName").val(name);
	 $.ajax({
        type:"POST",
        url :ctx+"/brand/display_linkageMainPageSecond",
        data:{
            id:id
        },
        dataType:"json",
