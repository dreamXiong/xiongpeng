...
<head>
	<meta charset="UTF-8">
	<c:set value="${serviceType.name}" var="titleName"></c:set>
...
<div class="container" style="padding-bottom:10px;">
	<div class="mylist mylist-down" style="padding-top:10px;">
		<ul class="box-shadow">
			<li id="time" class="make-time list-two">
				<a href="javascript:;">
					<img src="${ctx }/images/icon-16.png" alt="">
					<c:choose>
						<c:when test="${empty param.appointmentsStr}">
							<span id="appointmentsStrSpan">请选择预约时间</span>
						</c:when>
						<c:otherwise>
							<span id="appointmentsStrSpan">${param.appointmentsStr }</span>
						</c:otherwise>
					</c:choose>
					<i class="right"></i>
				</a>
		...
		<ul class="box-shadow" style="padding-top:10px;">
			...
			<li>                                        
				<div class="form-group">
					<input type="text" placeholder="请输入您设备的大致状况（选填）" value="${param.messageStr}" id="message">
...
<footer class="footer serve-foot">
	<button type="button" id="submitOrderButton">下单</button>
</footer>
<!-- 提交订单form -->
<form id="submitOrderForm" method="post" action="${ctx }/wapOrderService/submitOrder">
	...
	<input type="hidden" value="${serviceType.id }" id="serviceTypeId" name="serviceTypeId"/>
	<input type="hidden" value="${param.appointmentsStrId }" id="appointments" name="appointments"/>
	<input type="hidden" value="0" id="isSubmit"/>
</form>
<!-- 修改地址 -->
<form id="updateMyaddressForm" method="post" action="${ctx }/wapOrderService/showMyAddress">
	<input type="hidden" value="${wapAddress.id }" id="addressId" name="addressId"/>
	<input type="hidden" value="${serviceType.id }" id="serviceTypeId" name="serviceTypeId"/>
...
$(document).delegate('#submitOrderButton','click',function(event) { 
	if($("#submitOrderForm #isSubmit").val() == '0'){     
		...
		var appointments = $("#submitOrderForm #appointments").val();
