<%@page pageEncoding="UTF-8"%>
	<div class="screen">
		<header class="text-center">
			<span class="icon-angle-left text-center back"></span>筛选<a class='selectPickInfo'>确定</a>  
		</header>
		<div class="class-nav">
							...
							>
								...
								<ul class="clearfix" id="productActive">
									<c:forEach var="p" items="${item.wapProductTypeDtoList}" varStatus="wp">
											<li name="${p.secondId }"><a name="${p.secondId }">${p.secondType }</a></li>
									</c:forEach>
								</ul>
							</div>
						</c:forEach>
							...
							>
								...
								<ul class="clearfix" id="brandActive">
									<c:forEach var="b" items="${item.wapBrandDtoList}" varStatus="wb">
										<li name="${b.brandId}"><a name="${b.brandId}" >${b.brandName}</a></li>
									</c:forEach>
								</ul>
							</div>	
						</c:forEach>
